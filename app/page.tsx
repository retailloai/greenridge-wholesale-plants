  ...
  return (
    <div className={`relative overflow-hidden bg-gradient-to-br ${bg} ${height} ${hero ? "rounded-[44px]" : thumb ? "rounded-2xl" : "rounded-[26px]"}`}>
      ...
      {Array.from({ length: leaves }).map((_, i) => {
        ...
        return (
          <div key={i} className={`absolute left-1/2 origin-bottom rounded-[100%] bg-gradient-to-br ${colour} shadow-sm`} style={{ width: `${leafWidth}px`, height: `${leafHeight}px`, bottom: hero ? "156px" : thumb ? "44px" : compact ? "82px" : "96px", transform: `translateX(-50%) rotate(${angle}deg) translateY(-${hero ? 46 : thumb ? 14 : 24}px)`, opacity: 0.92 }} />
        );
      })}
      {product.theme === "flower" && !thumb && Array.from({ length: hero ? 12 : 7 }).map((_, i) => <div key={`f-${i}`} className="absolute z-10 rounded-full bg-white shadow-sm" style={{ width: hero ? 18 : 12, height: hero ? 18 : 12, left: `${38 + (i % 6) * 6}%`, top: `${32 + Math.floor(i / 6) * 8}%` }} />)}
...
function Hero({ approved }: { approved: boolean }) {
  return (
    <section id="home" className="relative isolate overflow-hidden bg-[#fbfaf5]">
    ...
    <section id="trade" className="bg-[#fbfaf5] px-5 py-14">
      <div className="mx-auto grid max-w-[1500px] gap-8 lg:grid-cols-[1fr_0.8fr]">
        ...
        <div className="rounded-[36px] bg-white p-7 shadow-sm ring-1 ring-stone-200">
          {submitted ? <div><div className="text-5xl">✅</div><h3 className="mt-3 font-serif text-3xl font-black text-emerald-950">Application received</h3><p className="mt-2 leading-7 text-stone-600">Your trade application has been saved. We will review it and contact you shortly.</p><button onClick={() => setApproved(true)} className="mt-5 w-full rounded-2xl bg-emerald-800 px-6 py-4 font-black text-white">Preview approved buyer catalogue</button></div> : <form onSubmit={handleSubmit}><h3 className="font-serif text-3xl font-black text-emerald-950">Trade Account Application</h3><p className="mt-1 text-stone-600">Apply for wholesale access in minutes.</p><div className="mt-5 grid gap-3"><input name="business_name" required placeholder="Business Name *" className="h-14 rounded-2xl border border-stone-200 bg-stone-50 px-4 font-semibold outline-none focus:ring-4 focus:ring-emerald-600/20" /><input name="abn" placeholder="ABN" className="h-14 rounded-2xl border border-stone-200 bg-stone-50 px-4 font-semibold outline-none focus:ring-4 focus:ring-emerald-600/20" /><input name="delivery_suburb" placeholder="Delivery Suburb" className="h-14 rounded-2xl border border-stone-200 bg-stone-50 px-4 font-semibold outline-none focus:ring-4 focus:ring-emerald-600/20" /><input name="expected_monthly_volume" placeholder="Expected Monthly Volume" className="h-14 rounded-2xl border border-stone-200 bg-stone-50 px-4 font-semibold outline-none focus:ring-4 focus:ring-emerald-600/20" /><input name="email" required type="email" placeholder="Email *" className="h-14 rounded-2xl border border-stone-200 bg-stone-50 px-4 font-semibold outline-none focus:ring-4 focus:ring-emerald-600/20" /><input name="phone" placeholder="Phone" className="h-14 rounded-2xl border border-stone-200 bg-stone-50 px-4 font-semibold outline-none focus:ring-4 focus:ring-emerald-600/20" /><select name="buyer_type" className="h-14 rounded-2xl border border-stone-200 bg-stone-50 px-4 font-semibold outline-none focus:ring-4 focus:ring-emerald-600/20"><option>Retail Store</option><option>Landscaper</option><option>Grower / Nursery</option><option>Interior Plant Stylist</option><option>Developer / Project Buyer</option></select><textarea name="message" placeholder="Message" className="min-h-24 rounded-2xl border border-stone-200 bg-stone-50 p-4 font-semibold outline-none focus:ring-4 focus:ring-emerald-600/20" />{status && <div className="rounded-2xl bg-emerald-50 p-4 text-sm font-black text-emerald-950">{status}</div>}<button disabled={saving} className="mt-2 rounded-2xl bg-emerald-800 px-6 py-4 font-black text-white disabled:opacity-60">{saving ? "Saving..." : "Apply for Wholesale Access"}</button></div><p className="mt-3 text-sm font-semibold text-stone-500">We typically respond within 1 business day.</p></form>}
        </div>
      </div>
    </section>
  );
...
export default function PremiumWholesalePlantDemo() {
  ...
  return (
    <div className="min-h-screen bg-[#fbfaf5] text-stone-900">
      ...
      <SeoAndDeploy />
      <footer id="contact" className="border-t border-stone-200 bg-white px-5 py-10"><div className="mx-auto grid max-w-[1500px] gap-6 md:grid-cols-3"><div><div className="font-serif text-3xl font-black text-emerald-950">Wholesale Green Co</div><p className="mt-2 text-sm leading-6 text-stone-600">Premium gated wholesale plant catalogue for Australian trade buyers.</p></div><div className="space-y-2 text-sm font-semibold text-stone-600"><div>☎️ Phone placeholder</div><div>✉️ Email placeholder</div><div>📍 Melbourne, Victoria</div></div><div className="rounded-2xl bg-stone-50 p-4 text-sm leading-6 text-stone-600">Product names, plant imagery and selected reference information are used with supplier approval and adapted for branding and marketing purposes.</div></div></footer>
