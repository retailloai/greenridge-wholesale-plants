...
const products = [
  { id: 1, name: "Sword Fern", botanical: "Nephrolepis exaltata", category: "Indoor Plants", type: "Fern", available: 500, cost: 0.96, price: 1.75, project: 1.65, description: "Lush, arching fronds with premium retail appeal for indoor spaces, shaded gardens and styling projects.", bestFor: ["Indoor", "Fern"], theme: "fern" },
  { id: 2, name: "White Agapanthus", botanical: "Agapanthus ‘Albus’", category: "Landscape & Outdoor", type: "Flowering", available: 500, cost: 0.96, price: 2.35, project: 2.15, description: "Clean white flowering stems for borders, mass planting and premium landscape projects.", bestFor: ["Outdoor", "Flowering"], theme: "flower" },
  { id: 3, name: "Chives", botanical: "Allium schoenoprasum", category: "Grasses & Groundcovers", type: "Herb", available: 1000, cost: 0.96, price: 1.45, project: 1.35, description: "A hardy culinary herb with delicate purple blooms and strong nursery appeal.", bestFor: ["Herb", "Edible"], theme: "grass" },
  { id: 4, name: "Chinese Evergreen", botanical: "Aglaonema modestum", category: "Indoor Plants", type: "Foliage", available: 1000, cost: 0.82, price: 2.10, project: 1.95, description: "Tough, adaptable foliage with strong variegation for indoor retail and interior plant programs.", bestFor: ["Indoor", "Foliage"], theme: "variegated" },
  { id: 5, name: "Cordyline Pink Diamond", botanical: "Cordyline fruticosa", category: "Indoor Plants", type: "Foliage", available: 1500, cost: 0.96, price: 2.00, project: 1.85, description: "A vibrant foliage line with strong decorative appeal for indoor plant ranges and retail displays.", bestFor: ["Indoor", "Colour"], theme: "pink" },
  { id: 6, name: "Ficus Ruby", botanical: "Ficus elastica", category: "Indoor Plants", type: "Feature", available: 1000, cost: 0.89, price: 1.85, project: 1.75, description: "Stylish indoor foliage with modern retail appeal and strong decorative value for premium buyers.", bestFor: ["Retail", "Feature"], theme: "variegated" },
  { id: 7, name: "Lomandra Lime Tuff", botanical: "Lomandra longifolia", category: "Landscape & Outdoor", type: "Landscape", available: 20000, cost: 0.96, price: 1.75, project: 1.65, description: "Large-volume landscape performer for commercial, council and development projects.", bestFor: ["Bulk", "Landscape"], theme: "grass" },
  { id: 8, name: "Monstera Deliciosa", botanical: "Monstera deliciosa", category: "Indoor Plants", type: "Feature", available: 500, cost: 0.96, price: 2.25, project: 2.10, description: "High-demand indoor favourite with strong appeal for retail, offices and interiors.", bestFor: ["Feature", "Interior"], theme: "broadleaf" },
  { id: 9, name: "Philodendron Birkin", botanical: "Philodendron birkin", category: "Indoor Plants", type: "Premium", available: 1000, cost: 0.85, price: 2.00, project: 1.85, description: "Premium patterned foliage with strong shelf appeal for modern plant buyers.", bestFor: ["Premium", "Patterned"], theme: "variegated" },
  { id: 10, name: "Syngonium Neon", botanical: "Syngonium hybrid", category: "Indoor Plants", type: "Colour", available: 1500, cost: 0.79, price: 1.70, project: 1.60, description: "Colourful foliage option with strong decorative appeal for indoor displays.", bestFor: ["Colour", "Indoor"], theme: "pink" },
  { id: 11, name: "Dianella Tazred", botanical: "Dianella hybrid", category: "Landscape & Outdoor", type: "Landscape", available: 2800, cost: 0.96, price: 1.75, project: 1.65, description: "Popular outdoor line with strong colour and landscape appeal for professional programs.", bestFor: ["Outdoor", "Bulk"], theme: "grass" },
  { id: 12, name: "Agapanthus Purple Cloud", botanical: "Agapanthus africanus", category: "Flowering & Colour", type: "Flowering", available: 500, cost: 0.96, price: 1.75, project: 1.65, description: "Reliable flowering landscape variety suited to borders and mass planting.", bestFor: ["Flowering", "Outdoor"], theme: "flower" }
];
...
function PlantVisual({ product, size = "card" }: { product: Product; size?: "hero" | "card" | "thumb" }) {
  const hero = size === "hero";
  const thumb = size === "thumb";
  const leaves = product.theme === "grass" ? 14 : product.theme === "flower" ? 10 : product.theme === "pink" ? 9 : 12;
  ...
  return (
    <div className={`relative overflow-hidden bg-gradient-to-br ${bg} ${hero ? "h-[360px] rounded-[38px]" : thumb ? "h-20 w-20 rounded-2xl" : "h-48 rounded-[24px]"}`}>
      <div className="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-white/70" />
      <div className="absolute -bottom-14 -left-12 h-44 w-44 rounded-full bg-emerald-950/10" />
      ...
      {Array.from({ length: leaves }).map((_, i) => {
        ...
        const leafHeight = hero ? 82 + (i % 3) * 15 : thumb ? 25 : 48 + (i % 3) * 8;
      ...
      {product.theme === "flower" && !thumb && Array.from({ length: hero ? 12 : 7 }).map((_, i) => (
        <div key={i} className="absolute z-10 rounded-full bg-white shadow-sm" style={{ width: hero ? 18 : 12, height: hero ? 18 : 12, left: `${37 + (i % 6) * 6}%`, top: `${34 + Math.floor(i / 6) * 8}%` }} />
      ))}
      <div className={`absolute left-1/2 -translate-x-1/2 bg-stone-950 shadow-2xl ${hero ? "bottom-16 h-22 w-28 rounded-b-3xl rounded-t-xl" : thumb ? "bottom-2 h-8 w-10 rounded-b-xl rounded-t-md" : "bottom-6 h-14 w-18 rounded-b-2xl rounded-t-lg"}`} />
...
function Header({ approved, setApproved, cartCount, setCartOpen }: { approved: boolean; setApproved: (v: boolean) => void; cartCount: number; setCartOpen: (v: boolean) => void }) {
  return (
...
function Hero() {
  return (
    <section id="home" className="relative overflow-hidden bg-[#fbfaf5]">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_75%_0%,rgba(22,101,52,0.16),transparent_34%),radial-gradient(circle_at_6%_45%,rgba(22,101,52,0.08),transparent_28%)]" />
      <div className="relative mx-auto grid max-w-[1500px] gap-8 px-5 pb-10 pt-12 lg:grid-cols-[0.9fr_1.1fr] lg:pt-16">
        <div className="z-10">
          ...
          <h1 className="font-serif text-5xl font-black leading-[0.94] tracking-tight text-emerald-950 md:text-6xl xl:text-7xl">Premium Wholesale<br />Plants for Trade Buyers</h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-stone-700">Beautiful indoor foliage and landscape varieties sourced from trusted growers and delivered for professional buyers.</p>
          ...
          <div className="mt-7 grid max-w-3xl gap-4 sm:grid-cols-3">
            {[["🌿","Quality Assured","Supplier approved & trade ready"],["🚚","Reliable Supply","Consistent availability all year round"],["🪴","Trade Focused","Built for landscapers, retailers & designers"]].map(([icon,title,text]) => (
              <div key={title} className="flex gap-3">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xl">{icon}</div>
                <div><div className="text-sm font-black text-emerald-950">{title}</div><div className="text-xs font-semibold leading-5 text-stone-600">{text}</div></div>
              </div>
            ))}
          </div>
        </div>
        <div className="relative min-h-[440px] overflow-hidden rounded-[46px] bg-gradient-to-br from-white via-emerald-50 to-emerald-100 p-8 shadow-sm ring-1 ring-stone-200">
          <div className="absolute -right-12 top-0 h-[92%] w-[50%] rounded-l-[90px] bg-emerald-900/12" />
          <div className="absolute left-10 top-20 h-80 w-80 rounded-full bg-emerald-900/10" />
          <div className="relative ml-auto max-w-[560px]">
            <PlantVisual product={products[0]} size="hero" />
          </div>
          <div className="absolute right-12 top-20 rounded-[26px] border border-white/60 bg-white/75 p-6 shadow-xl backdrop-blur">
      ...
      <div id="collections" className="relative mx-auto grid max-w-[1500px] gap-4 px-5 pb-8 lg:grid-cols-2">
        <div className="overflow-hidden rounded-[26px] bg-white p-6 shadow-sm ring-1 ring-stone-200">
          <div className="flex items-center gap-5">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-emerald-50 text-3xl">🪴</div>
            <div><h2 className="font-serif text-3xl font-black text-emerald-950">Indoor & Feature</h2><p className="mt-1 text-sm leading-6 text-stone-700">Stylish foliage and statement plants perfect for interiors and retail.</p></div>
          </div>
        </div>
        <div className="overflow-hidden rounded-[26px] bg-emerald-800 p-6 text-white shadow-sm">
          <div className="flex items-center gap-5">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-white/20 text-3xl">🌾</div>
            <div><h2 className="font-serif text-3xl font-black">Landscape & Outdoor</h2><p className="mt-1 text-sm leading-6 text-emerald-50">Hardy, reliable varieties for gardens, public spaces and landscapes.</p></div>
          </div>
        </div>
      </div>
    </section>
...
function ProductCard({ product, approved, addToCart, openProduct }: { product: Product; approved: boolean; addToCart: (p: Product, q?: number) => void; openProduct: (p: Product) => void }) {
  return (
    <article className="group overflow-hidden rounded-[24px] bg-white shadow-sm ring-1 ring-stone-200 transition hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="p-5">
        ...
        <div className="mt-3 flex flex-wrap gap-2">{product.bestFor.map((tag) => <span key={tag} className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-800">{tag}</span>)}</div>
        {approved ? (
          <div className="mt-5 grid grid-cols-2 gap-3">
            <div><div className="text-xs font-black text-stone-500">Trade price</div><div className="text-2xl font-black text-emerald-950">${product.price.toFixed(2)}</div><div className="text-xs font-semibold text-stone-500">each</div></div>
            <div><div className="text-xs font-black text-stone-500">Project from</div><div className="text-2xl font-black text-emerald-950">${product.project.toFixed(2)}</div><div className="text-xs font-semibold text-stone-500">each</div></div>
          </div>
        ) : (
...
function Catalogue({ approved, addToCart, openProduct }: { approved: boolean; addToCart: (p: Product, q?: number) => void; openProduct: (p: Product) => void }) {
    ...
    <section id="catalogue" className="bg-[#fbfaf5] px-5 py-12">
      <div className="mx-auto max-w-[1500px]">
        ...
        <div className="mb-5 grid gap-3 lg:grid-cols-[1fr_auto]">
          ...
          <select value={sort} onChange={(e) => setSort(e.target.value)} className="h-16 rounded-3xl border border-stone-200 bg-white px-6 text-base font-black text-emerald-950 outline-none shadow-sm">
            <option>Popularity</option><option>Availability</option><option>Price Low</option>
          </select>
        </div>
        ...
        <div className="mb-8 grid gap-3 rounded-3xl bg-emerald-50 p-4 text-sm font-bold text-emerald-950 md:grid-cols-4">
          <div>🪴 Premium quality, professionally grown</div><div>🔐 Trade prices for approved buyers</div><div>🚚 Delivered to your project</div><div>🏷️ 5,000+ mixed orders eligible</div>
        </div>
...
function TradeAccess({ setApproved }: { setApproved: (v: boolean) => void }) {
    ...
    <section id="trade" className="bg-[#fbfaf5] px-5 py-14">
      <div className="mx-auto grid max-w-[1500px] gap-8 lg:grid-cols-[1fr_0.8fr]">
        <div className="rounded-[42px] bg-emerald-950 p-8 text-white shadow-2xl md:p-10">
          ...
          <p className="mt-5 max-w-3xl text-lg leading-8 text-emerald-50/85">This protects sensitive wholesale prices while allowing customers to see your premium range, professional presentation and availability before applying.</p>
          <div className="mt-8 grid gap-4 md:grid-cols-2">{["Exclusive wholesale pricing","Priority stock availability","Dedicated trade support","Early access to new releases"].map((x) => <div key={x} className="rounded-3xl bg-white/10 p-5 font-bold">✅ {x}</div>)}</div>
        </div>
...
function ProductModal({ product, approved, close, addToCart }: { product: Product | null; approved: boolean; close: () => void; addToCart: (p: Product, q?: number) => void }) {
      ...
      <div className="max-h-[92vh] w-full max-w-6xl overflow-auto rounded-[42px] bg-[#fbfaf5] p-5 shadow-2xl">
        ...
        <div className="grid gap-7 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="rounded-[34px] bg-white p-7 shadow-sm ring-1 ring-stone-200">
            ...
            <div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-stone-50 p-4"><div className="text-xs font-black uppercase text-stone-500">Available</div><div className="text-2xl font-black text-emerald-950">{product.available.toLocaleString()}</div></div><div className="rounded-2xl bg-stone-50 p-4"><div className="text-xs font-black uppercase text-stone-500">Best for</div><div className="text-sm font-black text-emerald-950">{product.bestFor.join(", ")}</div></div></div>
            {approved ? <div className="mt-5 rounded-3xl bg-emerald-950 p-5 text-white"><div className="text-sm text-emerald-100">Wholesale Trade Price</div><div className="mt-1 text-4xl font-black">${product.price.toFixed(2)} AUD</div><p className="mt-2 text-sm text-emerald-100">Project pricing from ${product.project.toFixed(2)} each.</p></div> : <div className="mt-5 rounded-3xl bg-emerald-50 p-5 font-black text-emerald-950">🔒 Apply for trade access to unlock price.</div>}
            <div className="mt-5 flex gap-3"><button onClick={() => setQty(Math.max(50, qty-50))} className="h-14 w-14 rounded-2xl bg-stone-100 text-2xl font-black">−</button><input value={qty} onChange={(e)=>setQty(Number(e.target.value||0))} className="h-14 w-28 rounded-2xl border border-stone-200 text-center font-black"/><button onClick={() => setQty(qty+50)} className="h-14 w-14 rounded-2xl bg-stone-100 text-2xl font-black">+</button><button onClick={() => approved ? addToCart(product, qty) : document.getElementById("trade")?.scrollIntoView({behavior:"smooth"})} className="flex-1 rounded-2xl bg-emerald-800 px-6 py-4 font-black text-white">{approved ? `Add ${qty} to Bulk Order` : "Apply for Wholesale Access"}</button></div>
          </div>
        </div>
      </div>
    </div>
...
function CartDrawer({ cart, close, updateQty, removeItem, submitQuote, approved, savingQuote }: { cart: CartItem[]; close: () => void; updateQty: (id: number, qty: number) => void; removeItem: (id: number) => void; submitQuote: () => void; approved: boolean; savingQuote: boolean }) {
    ...
    <div className="min-h-screen bg-[#fbfaf5] text-stone-900">
      ...
      <footer id="contact" className="border-t border-stone-200 bg-white px-5 py-10">
        <div className="mx-auto grid max-w-[1500px] gap-6 md:grid-cols-3">
          <div><div className="font-serif text-3xl font-black text-emerald-950">Wholesale Green Co</div><p className="mt-2 text-sm leading-6 text-stone-600">Premium gated wholesale plant catalogue for Australian trade buyers.</p></div>
          <div className="space-y-2 text-sm font-semibold text-stone-600"><div>☎️ Phone placeholder</div><div>✉️ Email placeholder</div><div>📍 Melbourne, Victoria</div></div>
      ...
      {cartOpen && <CartDrawer cart={cart} close={()=>setCartOpen(false)} updateQty={updateQty} removeItem={removeItem} submitQuote={submitQuote} approved={approved} savingQuote={savingQuote} />}
